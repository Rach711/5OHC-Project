...
printf "1\n1\n" | gmx_mpi pdb2gmx -f $INPUT_PDB -o processed.gro -p topol.top -i posre.itp
...
gmx_mpi grompp -f $RESOURCES/ions.mdp -c solv.gro -p topol.top -o ions.tpr
...
gmx_mpi grompp -f $RESOURCES/em.mdp -c $TOPOL/solv_ions.gro -p $TOPOL/topol.top -o em.tpr
...
gmx_mpi grompp -f $RESOURCES/nvt.mdp -c $EM/em.gro -r $EM/em.gro -p $TOPOL/topol.top -o nvt.tpr
...
gmx_mpi grompp -f $RESOURCES/npt.mdp -c $NVT/nvt.gro -r $NVT/nvt.gro -t $NVT/nvt.cpt -p $TOPOL/topol.top -o npt.tpr
...
gmx_mpi grompp -f $RESOURCES/md.mdp -c $NPT/npt.gro -t $NPT/npt.cpt -p $TOPOL/topol.top -o md.tpr

# Run with full GPU offloading constraints natively on your local card
gmx_mpi mdrun -v -deffnm md -nb gpu -pme gpu -bonded gpu -update gpu -dlb yes -nsteps 5000
